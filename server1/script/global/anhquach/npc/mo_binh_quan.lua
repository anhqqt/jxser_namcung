...
function main()
	...
	if (BT_GetGameData(GAME_BATTLEID) ~= BT_GetData(PL_BATTLEID) or BT_GetGameData(GAME_BATTLESERIES) ~= BT_GetData(PL_BATTLESERIES)) then
		...
		if (bt_ncamp == 1) then
			Say("ChiÕn dŞch [" .. battlename .. "]ChiÕn dŞch ®· b¾t ®Çu, mäi ng­êi h·y v× tö d©n ®¹i tèng, tôc ng÷ cã c©u: Thiªn h¹ h­ng vong, thÊt phu h÷u tr¸ch. Nay ng­êi kim x©m ph¹m bê câi chóng ta, ®©y lµ lóc b¸o hiÕu ®Êt n­íc, chØ cÇn cÊp 40 trë lªn vµ cÇn 1Tèng Kim Chiªu Binh LÖnh Bµi th× cã thÓ ®Òn ®¸p quèc gia råi, nµo ng­êi anh em cßn chÇn chõ g× n÷a!", 
				2, 
				"Ta tham gia! (§iÓm tİch lòy trë vÒ 0) /bt_joinsong", 
				"§Ó ta suy nghÜ l¹i!/bt_oncancel")
		else
			Say("ChiÕn dŞch [" .. battlename .. "]ChiÕn dŞch ®· b¾t ®Çu, hìi c¸c vŞ dòng sÜ Kim quèc, thêi kh¾c nhÊt thèng thiªn h¹ vµ dÑp bän Tèng quèc nam man c¶n ®­êng cña chóng ta ®· ®Õn. Nay Kim quèc rÊt cÇn sù trî lùc cña c¸c ng­¬i, chØ cÇn cÊp 40 trë lªn vµ nép 1 Tèng Kim Chiªu Binh LÖnh Bµi lµ cã thÓ ®Òn ®¸p quèc gia råi, nµo ng­êi anh em cßn chÇn chõ g× n÷a!", 
				2, 
				"Ta tham gia! (§iÓm tİch lòy trë vÒ 0) /bt_joinjin", 
				"§Ó ta suy nghÜ l¹i!/bt_oncancel")
		end
		SubWorld = nOldSubWorld
		return
	...
	----------------------------------------------------------------------
	-- C¸c ®iÒu kiÖn ®Ó ®¨ng kı chİnh thøc lµ:
	-- 1. §· ®¨ng kı cho trËn chiÕn nµy
	-- 2. §· thuéc vÒ phe cña trËn chiÕn nµy
	-- 3. Phï hîp víi cÊp ®é chiÕn tr­êng cña trËn chiÕn hiÖn t¹i

	-- §¨ng kı chİnh thøc ®· më

	-- Storm tham gia thö th¸ch
	say_index = 1
	storm_ask2start(1)
end

-- §èi tho¹i §¹i chiÕn Tèng Kim b×nh th­êng
...
function bt_enterbattle()
	...
	if nWeekDay == 2 or nWeekDay == 4 or nWeekDay == 6 then
		...
		if( nHour >= 2045 and nHour < 2300)then
			...
			local szTong = GetTong() -- KiÓm tra cã trong bang héi hay kh«ng
			if szTong ~= nil and szTong ~= "" then
				if 2 == bt_ncamp then
					if GetCityOwner(4) ~= szTong and GetCityOwner(7) == szTong then -- 4 lµ BiÖn Kinh, 7 lµ L©m An, 2 lµ bªn Kim 
						Msg2Player("Bang héi chiÕm thµnh L©m An chØ ®­îc b¸o danh bªn Tèng!")
						SubWorld = nOldSubWorld
						return
					end
				elseif 1 == bt_ncamp then
					if GetCityOwner(4) == szTong and GetCityOwner(7) ~= szTong then -- 4 lµ BiÖn Kinh, 7 lµ L©m An, 1 lµ bªn Tèng
