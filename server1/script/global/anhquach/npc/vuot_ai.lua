...
function dragon_join()
	...
	if (GetCash() < VUOT_AI_PHI_BAO_DANH) then
		Say("Tham gia nhiÖm vô “Th¸ch thøc thêi gian” cÇn <color=red>"..(VUOT_AI_PHI_BAO_DANH/10000).."v¹n l­îng<color>. ChuÈn bÞ ®ñ tiÒn råi h·y quay l¹i!", 0)
		return
	end
		...
		if (sub >= 0) then
			...
			if ( state == 1 and GetMSPlayerCount(MISSION_MATCH, 1) == 0 and blevel == map_isadvanced[map_map[i]] ) then
				...
				if ndate <= 22 and ndate >= 10 and blevel == 1 then
					SetMissionV(VARV_BATCH_MODEL,1) -- BËt V­ît ¶i Cao CÊp (¶i 30) - MËt Phßng Cöa ¶i + TiÓu NhiÕp NhÝ TrÇn
				else
					SetMissionV(VARV_BATCH_MODEL,0) -- BËt V­ît ¶i Th­êng
				end
				
				tbLog:PlayerActionLog("EventChienThang042011","BaoDanhVuotAi") -- NhËt ký ®¨ng ký			
				return
			end
		end
	end
