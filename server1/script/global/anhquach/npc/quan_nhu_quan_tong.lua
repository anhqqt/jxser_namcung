...
function exp_exchange()
	...
	if( GetLevel() < 40 ) then
		...
		if (GetTiredDegree() == 2) then
			...
			-- Thêi gian ho¹t ®éng (25/09/2009 ®Õn 01/11/2009) tõ 12h30 ®Õn 23h30
...
function exp_check()
	if( GetLevel() < 40 ) then
		...
		Say("Qu©n Nhu quan: Ng­¬i cßn <color=green>"..expNgayConLai.."<color> ®iÓm kinh nghiÖm cã thÓ ®æi trong ngµy", getn(tbOpt), tbOpt);
	end
end
...
function wantpay(mark)
	if (mark == 9999) then		-- Ho¸n ®æi tÊt c¶
		mark = nt_getTask(747)
	end
...
function nt_setTask(nTaskID, nTaskValue)
	...
	SyncTaskValue(nTaskID) -- §ång bé ®Õn client
end

-- LÊy tr¹ng th¸i nhiÖm vô
