...
function Task_NewVersionAward()
	...
	if (nNum - nCancelNum) == 10 then
		...
		tbAwardTemplet:GiveAwardByList({nExp = DA_TAU_EXP_10}, "PhÇn th­ëng kinh nghiÖm hoµn thµnh nhiÖm vô D· TÈu h»ng ngµy")
	end

	if (nNum - nCancelNum) == 20 then
		Msg2Player("H«m nay hoµn thµnh nhiÖm vô D· TÈu lÇn thø 30, nhËn ®­îc kinh nghiÖm thªm!");
		tbAwardTemplet:GiveAwardByList({nExp = DA_TAU_EXP_20}, "PhÇn th­ëng kinh nghiÖm hoµn thµnh nhiÖm vô D· TÈu h»ng ngµy")
	end

	if (nNum - nCancelNum) == 30 then
		Msg2Player("H«m nay hoµn thµnh nhiÖm vô D· TÈu lÇn thø 30, nhËn ®­îc kinh nghiÖm thªm!");
		tbAwardTemplet:GiveAwardByList({nExp = DA_TAU_EXP_30}, "PhÇn th­ëng kinh nghiÖm hoµn thµnh nhiÖm vô D· TÈu h»ng ngµy")
	end
	...
	if (nNum == 40 and nCancelNum) == 0 then
		...
		tbAwardTemplet:GiveAwardByList({nExp = DA_TAU_EXP_40}, "PhÇn th­ëng kinh nghiÖm khi hoµn thµnh nhiÖm vô D· TÈu h»ng ngµy")
	end	
...
function tasklink_entence()
    -- BiÕn cÊp ®é tèi thiÓu nhËn NV D· TÈu - Modified by Anh Quach (07-06-2025)
    if GetLevel() < DA_TAU_LV_MIN then
        Say("H·y luyÖn ®Õn cÊp " .. DA_TAU_LV_MIN .. " råi h·y ®Õn t×m ta.", 0);
        return
    end
    ...
    if (tl_gettaskcourse() == 3) then
        ...
        Say(":<enter>VŞ " .. GetPlayerSex() .. "®· hoµn thµnh <color=yellow>" .. myCountTimes .. "<color>, nhiÖm vô nµy ngµy h«m nay cã thÓ thùc hiÖn l¹i<color=yellow>" .. (40 - GetTask(ID_TASKLINK_LIMITNUM)) .. "<color>, ng­¬i cã muèn lµm n÷a kh«ng?", 
			2,
...
function Task_CancelConfirm()
    ...
    if (myTaskCancel == 0) then
        ...
        Say(
            ...
            3, "§óng, ta kh«ng muèn lµm nhiÖm vô quû qu¸i nµy ®©u/Task_NormalCancel",
        ...
        Say(" HiÖn t¹i b¹n cßn" .. myTaskCancel .. " sè lÇn c¬ héi hñy bá nhiÖm vô, b¹n x¸c ®Şnh hñy bá nhiÖm vô lÇn nµy ®óng kh«ng?",
            2, "§óng, ta kh«ng muèn lµm nhiÖm vô quû qu¸i nµy ®©u/#Task_Cancel(1)",
            "Uhm! §Ó ta suy nghÜ l¹i ®·/Task_Wait"
		);
...
function Task_Accept()
    ...
    if (myTaskType == 1) then
        GiveItemUI("NhiÖm vô t×m vËt phÈm", 
			" ña? Nh÷ng thø ta cÇn ng­¬i t×m ®­îc cho ta ch­a?",
            "Task_Accept_01", 
			"Task_Wait"
		);
    elseif (myTaskType == 2) then
        GiveItemUI("NhiÖm vô mua vËt phÈm", 
			" ña? Nh÷ng thø ta cÇn ng­¬i t×m ®­îc cho ta ch­a?",
            "Task_Accept_02", 
			"Task_Wait"
		);
    elseif (myTaskType == 3) then
        GiveItemUI("NhiÖm vô hiÓn thŞ vËt phÈm", 
			" ña? Nh÷ng thø ta cÇn ng­¬i t×m ®­îc cho ta ch­a?",
            "Task_Accept_03", 
			"Task_Wait"
		);
    ...
    if (myQuality == 0) then
        ...
        if (GetProductRegion() == "vn") then
            ...
            elseif (myGenre == 6 and myDetail == 1 and myParticular == 1475) then
                ...
                SyncItem(nItemIndex);
                WriteLog(format("[Log phÇn th­ëng chuçi nhiÖm vô D· TÈu]%s: Tµi kho¶n %s, nh©n vËt %s hoµn thµnh chuçi nhiÖm vô D· TÈu nhËn ®­îc ®iÓm Tİch lòy D· TÈu (®iÓm kinh nghiÖm: %d)", date(" [%y n¨m %m th¸ng %d ngµy  %H giê %M phót]"), GetAccount(), GetName(), GetTask(TSK_TASKLINK_SEANSONPOINT)))
...
function SelectAward_Money(nAward)
    ...
    if nAward >= 300000 then
        WriteLog(" [Ghi nhí chuçi nhiÖm vô]" .. date(" [%y n¨m %m th¸ng %d ngµy  %H giê %M phót]") .. " [Tµi kho¶n]" .. GetAccount() .. " [Nh©n vËt]" .. GetName() .. "Trong nhiÖm vô liªn tiÕp nhËn ®­îc<money>" .. nAward .. "</money>phÇn th­ëng lµ l­îng b¹c!");
    end
    -- KiÓm tra ho¹t ®éng "Hoµng chi ch­¬ng - §ªm Huy Hoµng"
    -- NÕu cã bËt trªn s3relay task nh­ng kh«ng trong thêi gian hoÆc kh«ng bËt trªn s3relay task th× x1, 
    -- Trong thêi gian th× x2
...
function SelectAward_Cancel(nAward)
    ...
    WriteLog(" [Ghi nhí nhËn phÇn th­ëng]" .. date(" [%yn¨m%mth¸ng%dngµy%Hgiê%Mphót%Sgi©y]") .. " [Tµi kho¶n]" .. GetAccount() .. " [Nh©n vËt]" .. GetName() .. "B¹n nhËn ®­îc c¬ héi hñy bá nhiÖm vô, hiÖn t¹i cßn cã thÓ c¬ héi hñy <" .. myCancel .. "> lÇn.");
...
function _CancelTaskDebug()
    ...
    if nOrgTask == nDecTask or nOrgTask == 0 then
        ...
        WriteLog(" [B¶ng nhiÖm vô ®· ghi nhí sai]" .. date(" [%yn¨m%mth¸ng%dngµy%Hgiê%Mphót%Sgi©y]") .. " [Tµi kho¶n]" .. GetAccount() .. " [Nh©n vËt]" .. GetName() .. "C¬ héi hñy bá kh«ng c©n b»ng, hiÖn t¹i c¬ héi hñy bá cßn <" .. nDecTask .. "> lÇn, phÇn c¬ héi hñy bá cßn <" .. nOrgTask .. ">.");
        return 0;
    end
