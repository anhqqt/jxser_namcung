...
function main()
    ...
    elseif CheckAccountMod() then
        ...
        local szTitle =
            "<npc>.<enter><color=yellow>Vâ L©m TruyÒn Kú<color><enter><color=red>C÷u Niªn T­¬ng Phïng<color>.<enter>Hoan nghªnh c¸c anh hïng hµo kiÖt... !"
        local tbOpt = { -- {"Hæ trî test Game", namcung2},
        {"Hñy trang bÞ khãa", deltem}, {"NhËn L¹i Vßng S¸ng Hæ trî t©n thñ", nhanvongsanght},
        {"§æi tªn nh©n vËt", game_spreader2}, -- {"§æi r¸c lÊy vò khÝ ngÉu nhiªn", doirac},
...
function dkgm9()
    Msg2Player("<color=yellow>HiÖn t¹i ®ang cã<color><color=green> " .. GetPlayerCount() ..
                   "<color> Ng­­êi Online !.")
end
...
function dkgm()
    ...
    if CalcFreeItemCellCount() >= 1 then
        local tbItem = 
        {
            {szName="LÖnh bµi GN", tbProp={6, 1, 4257, 1, 0, 0}, nBindState = -2},
            {szName="LÖnh bµi GM - 2025", tbProp={6, 1, 4378, 1, 0, 0}, nBindState = -2},
            {szName="MÆt N¹ GM", tbProp={0, 11, 367, 1, 0, 0}, nBindState = -2},
        }
        tbAwardTemplet:GiveAwardByList(tbItem, "VËt PhÈm GM")
    end
end
...
function deltem()
    ...
    GiveItemUI("Hñy vËt phÈm", "§¹i hiÖp h·y cÈn träng trong viÖc hñy vËt phÈm!", "DisposeConfirm",
        "onCancel", 1);
end
...
function DisposeConfirm(nCount)
    ...
    for i = 1, nCount do
        ...
        WriteLog(date("%Y%m%d %H%M%S") .. "\t" .. " GM Hñy Item " .. GetAccount() .. "\t" .. GetName() .. "\t" ..
                     " Huû item " .. strItem)
    end -- for
...
function game_spreader2()
    ...
    tinsert(tbOpt, {"§¨ng ký ®æi tªn nh©n vËt thu phÝ", dangkydoiten})
...
function about_rename_role()
    local strInfo =
        "<color=green>Ch©n Nhi:<color> Chµo b¹n! V× tªn nh©n vËt cña b¹n vµ ng­êi ch¬i kh¸c gièng nhau, cho nªn hÖ thèng ®· gióp b¹n ®æi tªn, b¹n cã thÓ söa tªn nh©n vËt cña b¹n tr­íc ®©y, cã thÓ kiÓm tra xem tªn ®ã cã dïng ®­îc kh«ng. Chó ý! B¹n chØ cã 1 c¬ héi duy nhÊt ®Ó ®æi tªn, xin h·y cÈn thËn tõng b­íc.";
    func_online_rename_role(strInfo);
end
function func_online_rename_role(strInfo)
    if (check_renamerole() == 1) then
        Say(strInfo, 3, "T×m hiÓu tªn nh©n vËt cßn sö dông ®­îc kh«ng/query_rolename",
            "Thay ®æi tªn nh©n vËt cña m×nh/change_rolename", "§Ó lÇn sau ta thay ®æi vËy/cancel");
    else
        deny()
    end
end
...
function dangkydoiten()
    if (GetBit(GetTask(TASKVALUE_BLEND), 1) == 1) then
        Say(
            "<color=green>Ch©n Nhi:<color> B¹n vÉn cßn 1 lÇn ®æi tªn nh©n vËt. H·y chän '§Æt l¹i tªn cho nh©n vËt' ®Ó ®æi tªn nh©n vËt",
            1, "§Ó ta kiÓm tra xem sao/cancel")
        return
    end
    Say(
        "<color=green>Ch©n Nhi:<color> Mçi lÇn ®æi tªn nh©n vËt cÇn <color=red>100 tiÒn ®ång<color>, ng­êi chuÈn bÞ xong ch­a?",
        2, "Ta ®· chuÈn bÞ xong/dongkymua", "Khi nµo ®ñ tiÒn ta sÏ quay l¹i sau!/cancel");
end
function dongkymua()
    buy_addibox_yes(100)
end
function buy_addibox_yes(nNedCount)
    ...
    if (nCount < nNedCount) then
        Say(
            "<color=green>Ch©n Nhi:<color> Kh¸ch quan ch­a ®ñ 100 tiÒn ®ång! Khi nµo cã ®ñ tiÒn h·y quay l¹i.",
            1, "§Ó ta kiÓm tra xem sao/cancel")
        return
    end
    ...
    Talk(1, "",
        "<color=green>Ch©n Nhi:<color> B¹n nhËn ®­îc 1 lÇn ®æi tªn nh©n vËt. H·y chän '§Æt l¹i tªn cho nh©n vËt' ®Ó ®æi tªn nh©n vËt");
end
function deny()
    Say(
        "<color=green>Ch©n Nhi:<color> Xin lçi! B¹n ph¶i mua lÇn ®æi tªn nh©n vËt råi h·y chän chøc n¨ng nµy!",
        1, "BiÕt råi!/cancel")
end

tbitemht = {
    [1] = {
        szName = "ThÇn Hµnh Phï",
        tbProp = {6, 1, 1266, 1, 0, 0},
        nCount = 1,
        nBindState = -2,
        nExpiredTime = 10080
    },
    [2] = {
        szName = "Håi thµnh phï (nhá)",
        tbProp = {6, 1, 1082, 0, 0},
        nCount = 1,
        nBindState = -2
    },
    --		[3]	={szName = "Trang BÞ Kim Phong", tbProp = {0,184}, nCount = 1,  nBindState = -2, nQuality = 1,nExpiredTime = 10080},
    --		[4]	={szName = "Trang BÞ Kim Phong", tbProp = {0,179}, nCount = 1,  nBindState = -2, nQuality = 1,nExpiredTime = 10080},
    --		[5]	={szName = "Trang BÞ Kim Phong", tbProp = {0,177}, nCount = 1,  nBindState = -2, nQuality = 1,nExpiredTime = 10080},
    --		[6]	={szName = "Trang BÞ Kim Phong", tbProp = {0,178}, nCount = 1,  nBindState = -2, nQuality = 1,nExpiredTime = 10080},
    --		[7]	={szName = "Trang BÞ Kim Phong", tbProp = {0,183}, nCount = 1,  nBindState = -2, nQuality = 1,nExpiredTime = 10080},
    --		[8]	={szName = "Trang BÞ Kim Phong", tbProp = {0,181}, nCount = 1,  nBindState = -2, nQuality = 1,nExpiredTime = 10080},
    --		[9]	={szName = "Trang BÞ Kim Phong", tbProp = {0,182}, nCount = 1,  nBindState = -2, nQuality = 1,nExpiredTime = 10080},
    --		[10]	={szName = "Trang BÞ Kim Phong", tbProp = {0,185}, nCount = 1,  nBindState = -2, nQuality = 1,nExpiredTime = 10080},
    --		[11]	={szName = "Trang BÞ Kim Phong", tbProp = {0,180}, nCount = 1,  nBindState = -2, nQuality = 1,nExpiredTime = 10080},
    [3] = {
        szName = "LÖnh Bµi T©n Thñ",
        tbProp = {6, 1, 4265, 1, 0, 0},
        nCount = 1,
        nBindState = -2
    }
