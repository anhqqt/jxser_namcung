...
function sf_winbouns(n_camp)
	...
	-- ChØ bªn Th¾ng cã tÝch lòy > 1000 míi nhËn ®­îc qu©n c«ng bµi Tèng Kim
...
function GameOver()
	...
	-- NÕu sè ng­êi trong mét phe trªn chiÕn tr­êng thÊp h¬n sè l­îng tèi thiÓu, th× kÕt thóc tû thÝ, phe nµo ®«ng ng­êi h¬n sÏ trùc tiÕp th¾ng
...
function sf_join(camp)
	...
	if (result == 0) then
		-- §æi phÝ b¸o danh vÒ 3000 l­îng - Modified by Anh Quach - 05062025
		...
		if (GetCash() >= SONGJIN_SIGNUP_FEES) then
			...
			local szMsg = "Xin lçi, tham gia ®ît Tèng Kim ®¹i chiÕn nµy, cÇn ph¶i nép phÝ b¸o danh lµ 3000 l­îng"
			Say(szMsg, 0)
			return
		end
	end
